...
async function main() {
    ...
    for (const item of data) {
        ...
        if (item.grupo === null) {
            ...
            for (const g of item.grupo) {
                ...
                if (!groups[g]) {
                    await cnn2.query("INSERT INTO `grupos`(`id`, `eid`, `titulo`, `descripcion`) VALUES (" + g + ",'" + item.id_equipo + "','" + g + "',NULL)")
                    groups[g] = true
                }
            }
        }
    ...
    for (const item of complementary) {
        ...
        await cnn1.query("INSERT INTO `complementarios`(`uid`, `talla`, `pais`, `ciudad`, `parroquia`, `coordenadas`, `profesion`, `sangre`, `terminos`) VALUES (" + id + "," + item.talla + ",'" + item.pais + "','" + item.ciudad + "','" + item.parroquia + "',NULL,'" + item.profesion + "',NULL,0)")
    }
