...
class AppModel {
    ...
    async getAllVariables(language) {
        return await AppModel.mysql.find("SELECT *, `" + language + "` as titulo, `sub" + language + "` as subtitulo FROM `diccionario` ORDER BY `abreviacion` ASC")
    }

    async getAllCollections(language) {
        return await AppModel.mysql.find("SELECT *, `" + language + "` as titulo FROM `colecciones`")
    }
    ...
    #variables = ["id", "es", "en", "subes", "suben", "bandera", "formato", "coleccion", "icono", "tipo", "abreviacion", "pattern", "posicion", "min", "max"]
    get variables() {
