...
class AdminModel {
    ...
    static async fromAuthenticator(loggerKey, loggerValue) {
        const data = await AdminModel.mysql.findOne("SELECT `id`,`equipo` FROM `administradores` WHERE `" + loggerKey + "` = '" + loggerValue + "'")
    ...
    static async fromAccess(accs) {
        ...
        const data = await AdminModel.mysql.findOne("SELECT `id`,`estado`,`equipo` FROM `administradores` WHERE `id` = " + aid)
    ...
    async variableList() {
        const result = await AdminModel.mongo.findOne({ _id: this.id }, "lista_variables")
        ...
        const response = entries.map(entry => parseInt(entry[0]))
