...
class AdminModel {
    ...
    async validatePatient(pid) {
        const result = await AdminModel.mysql.findOne("SELECT * FROM `pacientes` WHERE `pid` = " + pid + " AND `eid` = " + this.team)

        console.log("SELECT * FROM `pacientes` WHERE `pid` = " + pid + " AND `eid` = " + this.team)
