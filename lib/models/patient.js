...
class PatientModel {
    ...
    static async fromAuthenticator(loggerKey, loggerValue) {
        const data = await PatientModel.mysql.findOne("SELECT `id`,`estado`,`fecha_inicio` FROM `usuarios` WHERE `" + loggerKey + "` = '" + loggerValue + "'")
    ...
    static async fromAccess(accs) {
        ...
        const data = await PatientModel.mysql.findOne("SELECT `id`,`estado`,`fecha_inicio` FROM `usuarios` WHERE `id` = " + uid)
