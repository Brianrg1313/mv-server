...
class AdminModel {
    ...
    async groupsOfTeam() {
        return await AdminModel.mysql.find("SELECT `titulo`,`descripcion` FROM `grupos` WHERE `eid` = " + this.team)
    }

    async variablesOfTeam() {
        return await AdminModel.mysql.find("SELECT `titulo`,`color`,`icono` FROM `lista_variables` WHERE `eid` = " + this.team)
    }
