...
class PatientListModel {
     ...
     */
    async myPatients(aid, team, group) {
        let sql = "SELECT `pid` FROM `pacientes` WHERE `eid` = " + aid

        if (group !== null) {
            sql += " AND `gid` IN (" + group.join(",") + ")"
        }

        const result = await PatientListModel.adminDB.find(sql)

        if (!result) return null
        if (result.length === 0) return null

        const ids = result.map(item => parseInt(item.pid))

        const patients = await PatientListModel.patientDB.find("SELECT " + this.personalData.map(e => "`" + e + "`").join(",") + " FROM `usuarios` WHERE `estado` IS NULL AND `id` IN (" + ids.join(",") + ")")
