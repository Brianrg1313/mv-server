...
class Validador {
    ...
    #cleanRules(rules) {
        for (const rule in rules) {
            const minRule = rule.toLocaleLowerCase()
            if (!this.#mandatory.includes(minRule)) {
                if (rules[rule]) {
                    this.#mandatory.push(minRule)
                }
            }
    ...
    #validateText(k) {
        ...
        /// opcion a minusculas
        this.data[Validador.rules[k].key] = `${value}`.toLocaleLowerCase()
    }
