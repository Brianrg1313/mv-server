...
function filterByText(list, text) {
    ...
    for (const patient of list) {
        if (cleanText(`${patient.nombre} ${patient.apellido}`).includes(text)) {
            response.push(patient)
        } else if (cleanText(patient.usuario).includes(text)) {
            response.push(patient)
        } else if (cleanText(patient.correo).includes(text)) {
            response.push(patient)
        }
    }
