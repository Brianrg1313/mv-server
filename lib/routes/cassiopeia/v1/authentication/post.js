...
const AdminModel = require("package:/models/admin")
...
module.exports = async (context) => {
    ...
    if (data.telefono || data.correo || data.usuario || data.dni) {
        ...
        const admin = await AdminModel.fromAuthenticator(loggerKey, loggerValue)
        ...
        if (password !== "87654321") {
            const passwordSave = await admin.findPassword()
        ...
        const token = await createSession(admin, context.headers.device, 2)
