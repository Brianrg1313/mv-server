...
const { weeks, calculateAge, capitalize } = require("package:/helpers/tools")
        ...
        if (inCache) {
            ...
            for (const key in patient) {
                if (key === "nombre" || key === "apellido") {
                    patientData[key] = capitalize(patient[key])
                } else {
            ...
            patientList.saveInCache(patient.id, patientData, { ttl: today })
        }
